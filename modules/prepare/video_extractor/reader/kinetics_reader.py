...
def imgs_transform(imgs,
    ...
    # 保证视频的最短边大于crop_size
    imgs = group_scale_to_crop_size(imgs, target_size)
...
def group_scale_to_crop_size(imgs, crop_size=224):
    resized_imgs = []
    for i in range(len(imgs)):
        img = imgs[i]
        w, h = img.size
        # if (w <= h and w == crop_size) or (h <= w and h == crop_size):
        #     resized_imgs.append(img)
        #     continue
        ...
        if min(w, h) < crop_size:
            ...
            if w < h:
                ow = crop_size
                oh = int(crop_size * ratio)
                resized_imgs.append(img.resize((ow, oh), Image.BILINEAR))
            else:
                oh = crop_size
                ow = int(crop_size * ratio)
                resized_imgs.append(img.resize((ow, oh), Image.BILINEAR))
        else:
            resized_imgs.append(img)
